...
| Chart A | Ranked channels by offers per hour (warm/referral high; Easy Apply near zero) |
| Chart B | Where hires come from by source (volume vs efficiency) |
| Chart D | How many skill hours by level (entry / mid / senior) while you apply |
...
![Path: pick a job, show proof, then apply and learn in parallel, then practice interviews](assets/job-search-v8/path.png)
...
3d. Plan compensation and how you will negotiate an offer  
...
![This week: ask people you know, connect then message, apply on company site, one follow-up](assets/job-search-v8/this-week.png)
...
![Chart E score ladder: skip, apply plus one note, or full effort](assets/job-search-v8/chart-e.png)
...
![Company list tiers: stretch, stepping stone, sandbox](assets/job-search-v8/step1.png)
...
![Package: resume plus LinkedIn, proof projects, public GitHub or site](assets/job-search-v8/step2.png)
...
![Apply loop: find, verify, score, tailor, apply, track](assets/job-search-v8/step3a.png)
...
![Skills by level: entry more projects, mid one gap, senior almost no tutorials](assets/job-search-v8/step3b.png)
...
![Interview rounds: recruiter, timed SQL, stats or ML, business case, behavioral](assets/job-search-v8/step3c.png)
...
3. Use Stage 3 **Interview** worksheets (Interview Drills + Loop-Ready Pack) in Job Search  

**Starter SQL warm-up** (once, under a clock): Employee Salaries (15m), Average Population of Each Continent (20m), African Cities (25m) on HackerRank. Then rotate harder joins / windows from the Interview Prep SQL list.

Loops often run: recruiter → timed SQL → stats/ML judgment → business case → behavioral.

**Also pack:** 3 CAR behavioral stories + why-this-company + project walk-through (Loop-Ready Pack).
...
Decide your floor **before** money comes up. When someone asks expected salary, answer from research, not panic. When you have an offer, negotiate **once** on the total package. Walk away if it stays below your floor.

### Do this

1. Research market range; write floor + walk-away (Stage 4 Comp Planning)  
2. Practice expected-salary reply: deflect early → market range → floor-anchored ask → total package  
3. When you have an offer: ask once on total comp (base, bonus, equity, PTO, start, remote)  
4. Use must-haves; if below floor after one ask, walk away  
5. Every 20-30 qualified apps: Outcomes Review (channels), not just hope  

Use Stage 4 worksheets: **Compensation Planning** + **Search Outcomes Review**.
...
![Channel ladder: recruiter finds you at top, Easy Apply trap at bottom](assets/job-search-v8/channels.png)
