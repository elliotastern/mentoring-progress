# Chart E Role Scorecard — Worksheet

**Save a copy** in your mentorship Drive folder. Duplicate the score table for each role you consider.

**Purpose:** decide effort before you apply. Score 0 / 1 / 2 on five factors; total 0–10.

Guide: [Overview · Chart E](../01%20-%20Job%20Search%20Overview%20v8.md#before-you-apply--chart-e-effort-score-010)

---

## Role card

- Employer: __________  
- Role: __________  
- Careers-page URL (verified): __________  
- Date posted: __________  
- Source found: __________  

## Score (0 / 1 / 2 each)

| Factor | 0 | 1 | 2 | My score |
|--------|---|---|---|----------|
| Interest | Would not accept | Acceptable | Strongly want | |
| Core fit | Major gaps | Mostly qualified | Direct match | |
| Proof | Weak examples | Related evidence | Strong quantified evidence | |
| Human access | No contact | Possible contact | Warm / referral | |
| Logistics | Major conflict | Workable | Remote/part-time/pay fit | |
| **Total** | | | | **/10** |

## Effort rule

| Total | Action |
|------:|--------|
| 8–10 | Full effort: tailor, referral, HM/recruiter note, attach proof |
| 6–7 | Targeted apply + one concise outreach |
| 0–5 | Usually skip (unless intentional practice) |

My action for this role: __________

## Apply checklist (if continuing)

- [ ] Careers page verified (location, level, auth, pay/tools)  
- [ ] Résumé summary + first bullets tailored to **core** duties  
- [ ] Applied on employer site when practical  
- [ ] One short outreach sent (if 6+)  
- [ ] Logged in tracker with Chart E + follow-up date  

## Practice log (last 3 roles)

| Employer | Role | Chart E | Action |
|----------|------|--------:|--------|
| | | | |
| | | | |
| | | | |

## Done when

- [ ] I can score a new role in under 5 minutes  
- [ ] Progress Stage 3 answer = last Chart E total  
